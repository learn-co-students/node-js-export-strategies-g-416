...
exports.outage = function outage(device) {
  ...
  console.log(
    `Uh-oh, ${device.constructor.name}'s power is out. Try turning it on and checking its \`currentBrightness\`.`
  );
};
...
exports.surge = function surge(device) {
  if (device.currentBrightness > 0) {
    ...
    console.log(
      `${device.constructor.name} is surging! Current brightness: ${device.currentBrightness}`
    );
  }
};
